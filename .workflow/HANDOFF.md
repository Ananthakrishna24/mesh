...
- **P07 Linux CPU host proof** (`ebe19d8`, handoff `c3bf3fb`):
  ...
  - Durable cache kept at `$HOME/mesh-p07-smoke` (~7.6G, whole shards + HF sidecars).

# In progress
- Nothing mid-edit. Working tree clean on `c3bf3fb`. Next session starts at Next #1.
...
- Linux CPU generate is host evidence only; checklist CUDA/Metal proof boxes stay unchecked until those backends run.

# Gotchas
- Host has NVIDIA driver + `libcuda` + RTX 4070 SUPER, but **no CUDA toolkit/`nvcc`**. Default build is CPU. `--features cuda` needs toolkit installed.
...
- Qwen3-4B ~8GB FP16 weights; CPU F32 load uses more RAM; first cold generate is slow, cache-hit path is fast.
...
- Branch is **2 commits ahead of origin** (`ebe19d8`, `c3bf3fb`); push when ready.

# Next
1. Install CUDA toolkit on this Linux host (needs `nvcc` / toolkit, not just driver), then prove CUDA load/generate:
   ```bash
   # GUI path (reuses prepared weights):
   MESH_DATA_DIR=$HOME/mesh-p07-smoke cargo run -p mesh-app --release --features cuda
   # Select Qwen3-4B → Probe → Prepare (cache hit) → Load → Generate

   # Or gated smoke with CUDA-enabled mesh-node:
   ...
   ```
   Expect `backend=cuda` (FP16). Record under checklist P07 Linux CUDA proof.
2. Optional dual-window enrollment anytime (`/tmp/mesh-a`, `/tmp/mesh-b`).
3. Do **not** start P08 until a required backend proof is accepted; Linux CPU is not the CUDA checklist item.
4. After Linux CUDA: macOS Metal / Windows CUDA; then **P07.5**.
5. Push `ebe19d8` + `c3bf3fb` when ready.
...
| `docs/implementation/checklist.md` | Progress checkboxes + P07 CPU evidence note |
